...
- **nlm CLI** (optional) — NotebookLM CLI for source-grounded answering against a curated reference corpus. If installed, verify with `nlm doctor`; refresh auth with `nlm login`. Operator-specific notebook IDs and routing rules live in `CLAUDE.local.md` (gitignored, per-machine) — create one from your own corpus if you use this layer.
