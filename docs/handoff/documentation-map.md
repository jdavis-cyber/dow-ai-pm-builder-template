...
| What is the current phase/gate? | `.governance/gate_state.json` and `.governance/Phase_Gates/` | [Not Approved / Ready / Approved] |
...
A file's presence is not approval. Approval requires the relevant gate/decision record to show an approved status backed by the project's required approval mechanism. Scaffolded, copied, or historical artifacts must remain labeled as such.
