...
## 3. Gated Implementation Paths

Review `automation/gatekeeper.py` before implementation begins.

Default gated prefixes:
...
Default gated root/runtime files:
...
- [ ] Add project-specific gated paths if needed.
- [ ] Record any path changes in `docs/decisions/`.
- [ ] Verify closed-gate writes are blocked for source paths.
- [ ] Verify discovery/governance documentation writes remain allowed.
