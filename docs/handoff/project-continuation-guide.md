...
6. Check `.governance/gate_state.json` and `.governance/Phase_Gates/`.
7. Check `orchestration/tasks.md`.
8. Inspect `git status --short --branch` before changing files.
9. Do not write to gated implementation paths until the applicable phase gate is open by the approved mechanism.
...
|---|---|---|
| `.governance/gate_state.json` | Machine-readable gate state | Canonical state; do not edit manually |
| `.governance/Phase_Gates/` | Human-readable gate packages | Narrative evidence; approval depends on project mechanism |
...
- Edit `.governance/gate_state.json` manually.
...
```text
You are taking over [Project Name]. Start by reading PROJECT.md, docs/handoff/documentation-map.md, docs/handoff/project-continuation-guide.md, orchestration/system_spec.md, the active coordination file, directives, .governance/gate_state.json, and orchestration/tasks.md. Verify git status. Do not write to gated implementation paths until the active gate is open by the approved mechanism. Continue from [specific task/gate] and preserve evidence in docs/verification/.
```
